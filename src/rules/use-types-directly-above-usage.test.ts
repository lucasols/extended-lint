...
tests.addValid(
  'checkOnly generic-args - ignores types used outside generic arguments',
  `
    function processUser(data: UserData) {
      return data.name
    }
    ...
    type ButtonProps = { label: string }
  `,
  { checkOnly: ['generic-args'] },
)

tests.addValid(
  'checkOnly generic-args - allows types correctly placed above generic usage',
  `
    type FormConfig = {
      title: string
      show_title: boolean
    ...
    const formTextConfigSchema = rc_obj_builder<FormConfig>()({
      title: rc_string,
      show_title: rc_boolean,
      text: rc_string,
    })
...
tests.addValid(
  'checkOnly generic-args - ignores FC components even when used with generics',
  `
    const Button: React.FC<ButtonProps> = ({ label }) => {
      return <button>{label}</button>
    }
    ...
    const List: FC<ListProps<string>> = ({ items }) => {
...
tests.addInvalid(
  'checkOnly generic-args - catches type used in generic arguments',
  `
    const formTextConfigSchema = rc_obj_builder<FormConfig>()({
      title: rc_string,
      show_title: rc_boolean,
  ...
  `,
    appendToOutput: '\n\n',
    options: { checkOnly: ['generic-args'] },
  },
)

tests.addInvalid(
  'checkOnly generic-args - catches type used in function generic type parameter',
  `
    function fetchData<T>(): Promise<ApiResponse<T>> {
      return fetch('/api/data')
    }
...
tests.addInvalid(
  'checkOnly generic-args - catches type used in class generic',
  `
    class DataStore<T> extends BaseStore<StoreConfig<T>> {
      private data: T[]
...
tests.addValid(
  'checkOnly generic-args - ignores function arguments',
  `
    function processUser(data: UserData) {
      return data.name
    }
...
tests.addInvalid(
  'checkOnly mixed function-args and generic-args - catches both contexts',
  `
    function processUser(data: UserData) {
      return data.name
    }
    
    const formSchema = rc_obj_builder<FormConfig>()({
      title: rc_string,
    })
    
    type UserData = { name: string }
    type FormConfig = { title: string }
  `,
  [{ messageId: 'moveTypeAboveUsage' }],
  {
    output: `
    ...
    type FormConfig = { title: string }`,
    options: { checkOnly: ['function-args', 'generic-args'] },
  },
)
