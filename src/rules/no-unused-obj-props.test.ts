...
tests.describe('no unused return object props', () => {
  tests.addValid(
    'no unused return object props',
    `
      function getData() {
  ...
  tests.addInvalid(
    'partial usage of returned object is not valid in arrow function',
    `
      const getData = () => {
        return { id: 1, name: 'Test', value: 42 };
      }
    ...
    `,
    [
      { messageId: 'unusedReturnObjectProperty', data: { name: 'name' } },
      { messageId: 'unusedReturnObjectProperty', data: { name: 'value' } },
    ],
  )

  tests.addValid(
    'return object used with spread',
    `
      function getData() {
        return { id: 1, name: 'Test', value: 42 };
      }
  ...
  tests.addValid(
    'return object passed to function',
    `
      function getData() {
        return { id: 1, name: 'Test', value: 42 };
      }
  ...
  tests.addValid(
    'implicit return object in arrow function',
    `
      const getData = () => ({ id: 1, name: 'Test', value: 42 });
  ...
  tests.addInvalid(
    'unused property in returned object',
    `
      function getObj() {
        return { a: 1, b: 2 };
      }
    ...
    `,
    [{ messageId: 'unusedReturnObjectProperty', data: { name: 'b' } }],
  )

  tests.addInvalid(
    'unused property in inline returned object',
    `
      function getData() {
        return { id: 1, name: 'Test', unused: true };
      }

      const data = getData();
      console.log(data.id, data.name);
    `,
    [{ messageId: 'unusedReturnObjectProperty', data: { name: 'unused' } }],
  )

  tests.addInvalid(
    'multiple unused properties in returned object',
    `
      function getData() {
        return { id: 1, name: 'Test', value: 42, extra: 'unused', debug: true };
      }
    ...
    [
      { messageId: 'unusedReturnObjectProperty', data: { name: 'name' } },
      { messageId: 'unusedReturnObjectProperty', data: { name: 'value' } },
      { messageId: 'unusedReturnObjectProperty', data: { name: 'extra' } },
      { messageId: 'unusedReturnObjectProperty', data: { name: 'debug' } },
    ],
  )

  tests.addInvalid(
    'unused property in arrow function return',
    `
      const getData = () => ({ id: 1, name: 'Test', debug: true });
  ...
  tests.addValid(
    'using object destructuring with all properties used',
    `
      const getData = () => ({ id: 1, name: 'Test' });

      const { id, name } = getData();
      console.log(id, name);
    `,
  )

  tests.addValid(
    'ignore functions with explicit reference types with unknown properties',
    `
      const getData = (): Test => ({ id: 1, name: 'Test' });
  ...
  tests.addValid(
    'ignore functions with explicit types',
    `
      const getData: Fn = () => ({ id: 1, name: 'Test' });
  ...
  tests.addInvalid(
    'do not ignore functions with explicit object literal types',
    `
      const getData: Fn = (): { id: number; name: string } => ({ id: 1, name: 'Test' });
