...
export const noUnusedObjProps = createExtendedLintRule<
  [],
  'unusedObjectProperty' | 'unusedReturnObjectProperty'
>({
  name: 'no-unused-obj-props',
  meta: {
    type: 'suggestion',
    docs: {
      description: 'Disallow unused properties when it is safe to remove them',
    },
    messages: {
      unusedObjectProperty: 'The object property "{{name}}" is not being used',
      unusedReturnObjectProperty:
        'The object property "{{name}}" returned by this function is not being used',
    },
    schema: [],
  },
  defaultOptions: [],
  create(context) {
    return {
      VariableDeclarator(node) {
        ...
        if (node.parent.parent.type === AST_NODE_TYPES.ExportNamedDeclaration) {
          return
        }
        ...
        for (const ref of references) {
          ...
          if (ref.identifier.parent.type !== AST_NODE_TYPES.MemberExpression) {
            // If it's not a property access, mark the object as having non-property access
            return
          }

          // eslint-disable-next-line @typescript-eslint/consistent-type-assertions -- safe cast after type check above
